    ...
    public static class SeedData
    {
        public static async Task InitializeAsync(IServiceProvider serviceProvider)
        {
           ...
           if (context.Users.Any(u => u.UserName == "admin"))
             ...
             await _roleManager.CreateAsync(new IdentityRole("Gestor"));

            // User Gestor
            var gestor = new ApplicationUser
            {
                UserName = "gestor@gam.com",
                Email = "gestor@gam.com"
            };

            string gestorPWD = "Gestor123!";

            var createGestor = await _userManager.CreateAsync(gestor, gestorPWD);

            if (createGestor.Succeeded)
            {
                await _userManager.AddToRoleAsync(gestor, "Gestor");
            }
